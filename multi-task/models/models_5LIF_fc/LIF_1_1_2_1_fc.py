# -*- coding: utf-8 -*-
"""
LIF_1_1_2_1 model: 1+1+2+1 LIF topology for multi-task learning (mem-mixing design,
5 LIF slots).

Topology
--------
- Stage 1: 1 LIF processes the raw input image (slot 0)
- Stage 2: 1 LIF (slot 1) sees OLD mem[:,:,0] via lif_fc1 = nn.Linear(1, 1)
- Stage 3: 2 LIFs (slots 2, 3) see OLD mem[:,:,1] via lif_fc2 = nn.Linear(1, 2)
- Stage 4: 1 LIF (slot 4) sees OLD (mem[:,:,2], mem[:,:,3])
  via lif_fc3 = nn.Linear(2, 1)
