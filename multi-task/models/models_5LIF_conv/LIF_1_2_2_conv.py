...
class lif_1_2_2(nn.Module):
    """
    1+2+2 LIF topology (conv variant, mem-mixing, 5 LIF slots).
    """
...
class SCNN_Model_LIF_1_2_2(nn.Module):
    def __init__(self, n_tasks):
        super(SCNN_Model_LIF_1_2_2, self).__init__()
        self.n_tasks = n_tasks
        for i in range(self.n_tasks):
            setattr(self, 'task_{}'.format(i), nn.Linear(50, 10))

        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[0]
        self.lif_5_1 = lif_1_2_2(in_planes, out_planes, kernel_size, stride, padding)
        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[1]
        self.lif_5_2 = lif_1_2_2(in_planes, out_planes, kernel_size, stride, padding)
