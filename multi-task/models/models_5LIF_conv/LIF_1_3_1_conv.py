...
class lif_1_3_1(nn.Module):
    """
    1+3+1 LIF topology (conv variant, mem-mixing, 5 LIF slots).

    Slot layout: [batch, C', H', W', 5]
        slot 0     -> 1 primary LIF
        slots 1-3  -> 3 mixing LIFs from mem[:,:,:,:,0:1] via Linear(1, 3)
        slot 4     -> 1 mixing LIF from (mem[:,:,:,:,1], mem[:,:,:,:,2], mem[:,:,:,:,3])
                      via Linear(3, 1)
    """
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding):
        ...
        self.lif_fc1 = nn.Linear(1, 3).to(device)
...
class SCNN_Model_LIF_1_3_1(nn.Module):
    def __init__(self, n_tasks):
        super(SCNN_Model_LIF_1_3_1, self).__init__()
        self.n_tasks = n_tasks
        for i in range(self.n_tasks):
            setattr(self, 'task_{}'.format(i), nn.Linear(50, 10))

        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[0]
        self.lif_5_1 = lif_1_3_1(in_planes, out_planes, kernel_size, stride, padding)
        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[1]
        self.lif_5_2 = lif_1_3_1(in_planes, out_planes, kernel_size, stride, padding)
