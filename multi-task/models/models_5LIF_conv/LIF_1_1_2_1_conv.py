...
class lif_1_1_2_1(nn.Module):
    """
    1+1+2+1 LIF topology (conv variant, mem-mixing, 5 LIF slots).
    """
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding):
        super(lif_1_1_2_1, self).__init__()
        ...
        self.lif_fc2 = nn.Linear(1, 2).to(device)
        self.lif_fc2.weight.data = abs(self.lif_fc2.weight.data)
        self.lif_fc3 = nn.Linear(2, 1).to(device)
    ...
    def forward(self, input, mem, spike, is_spike_input=True):
        ...
        # Stage 3: 2 mixing LIFs from mem_a
        ...
        # Stage 4: 1 mixing LIF from (mem_b, mem_c)
        inner3 = self.lif_fc3(mem[:,:,:,:,2:4])
        mem_d, spike_d = mem_update(inner3[:,:,:,:,0], mem[:,:,:,:,4], spike[:,:,:,:,4])
...
class SCNN_Model_LIF_1_1_2_1(nn.Module):
    def __init__(self, n_tasks):
        super(SCNN_Model_LIF_1_1_2_1, self).__init__()
        self.n_tasks = n_tasks
        for i in range(self.n_tasks):
            setattr(self, 'task_{}'.format(i), nn.Linear(50, 10))

        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[0]
        self.lif_5_1 = lif_1_1_2_1(in_planes, out_planes, kernel_size, stride, padding)
        in_planes, out_planes, stride, padding, kernel_size = cfg_cnn[1]
        self.lif_5_2 = lif_1_1_2_1(in_planes, out_planes, kernel_size, stride, padding)
