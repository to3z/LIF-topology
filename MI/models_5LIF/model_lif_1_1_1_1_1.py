...
class LIF_1_1_1_1_1_neuron(nn.Module):
    """
    1+1+1+1+1 LIF topology (5-stage chain, mem-mixing, LIF_hh style, 5 LIF slots).

    Slot layout: [batch, channel, 5] (or [T, batch, channel, 5])
        slot 0 -> 1 primary LIF (processes input)
        slot k -> 1 stage-(k+1) mixing LIF, sees OLD mem[:,:,k-1] via
                  lif_fc_k = nn.Linear(1, 1) (non-negative scalar), k=1..4
    """
