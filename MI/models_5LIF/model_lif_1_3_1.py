...
class LIF_1_3_1_neuron(nn.Module):
    """
    1+3+1 LIF topology (mem-mixing, LIF_hh style, 5 LIF slots).

    Slot layout: [batch, channel, 5] (or [T, batch, channel, 5])
        slot 0     -> 1 primary LIF (processes input)
        slots 1-3  -> 3 stage-2 mixing LIFs, see OLD mem[:,:,0] via
                      lif_fc1 = nn.Linear(1, 3) (non-negative)
        slot 4     -> 1 stage-3 mixing LIF, sees OLD (mem[:,:,1], mem[:,:,2], mem[:,:,3])
