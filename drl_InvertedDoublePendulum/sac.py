...
from models.model_basic import DeterministicPolicy
from utils import soft_update, hard_update
from models.model_basic import QNetwork

class SAC(object):
    def __init__(self, num_inputs, action_space, args, GaussianPolicy):

        self.GaussianPolicy = GaussianPolicy
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = self.GaussianPolicy(num_inputs, action_space.shape[0], args.hidden_size, action_space).to(self.device)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        need_Qnetwork_seq = False
        if(need_Qnetwork_seq):
            state_batch = torch.FloatTensor(state_batch).to(self.device)
            next_state_batch = torch.FloatTensor(next_state_batch).to(self.device)
            action_batch = torch.FloatTensor(action_batch).to(self.device)
            reward_batch = torch.FloatTensor(reward_batch[:,:,-1,...]).to(self.device).unsqueeze(1)
            mask_batch = torch.FloatTensor(mask_batch[:,:,-1,...]).to(self.device).unsqueeze(1)
        else:
            state_batch = torch.FloatTensor(state_batch).to(self.device)
            next_state_batch = torch.FloatTensor(next_state_batch).to(self.device)
            action_batch = torch.FloatTensor(action_batch[:,-1,...]).to(self.device)
            reward_batch = torch.FloatTensor(reward_batch[:,:,-1,...]).to(self.device).unsqueeze(1)
            mask_batch = torch.FloatTensor(mask_batch[:,:,-1,...]).to(self.device).unsqueeze(1)
