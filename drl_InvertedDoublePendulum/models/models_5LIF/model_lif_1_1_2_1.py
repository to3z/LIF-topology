...
class LIF_1_1_2_1_neuron(nn.Module):
    """
    1+1+2+1 LIF topology (mem-mixing, LIF_hh style).

    Slot layout: [batch, channel, 5]
        slot 0          -> 1 primary LIF (processes input)
        slot 1          -> 1 stage-2 mixing LIF, sees OLD mem[:,:,0] via
                           lif_fc1 = nn.Linear(1, 1) (non-negative)
        slots 2, 3      -> 2 stage-3 mixing LIFs, see OLD mem[:,:,1] via
                           lif_fc2 = nn.Linear(1, 2) (non-negative)
        slot 4          -> 1 stage-4 mixing LIF, sees OLD (mem[:,:,2], mem[:,:,3])
                           via lif_fc3 = nn.Linear(2, 1) (non-negative)
    """
    def __init__(self, in_planes, out_planes):
        super(LIF_1_1_2_1_neuron, self).__init__()
        self.fc1 = nn.Linear(in_planes, out_planes)
        self.lif_fc1 = nn.Linear(1, 1)
        self.lif_fc1.weight.data = abs(self.lif_fc1.weight.data)
        self.lif_fc2 = nn.Linear(1, 2)
        self.lif_fc2.weight.data = abs(self.lif_fc2.weight.data)
        self.lif_fc3 = nn.Linear(2, 1)
    ...
    def update_neuron(self, input, mem, spike):
        ...
        input_all[:, :, 2] = inner2[:, :, 0]
        input_all[:, :, 3] = inner2[:, :, 1]
        input_all[:, :, 4] = self.lif_fc3(mem[:, :, 2:4]).squeeze(-1)
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        ...
        self.lif_1_1_2_1_layer1 = LIF_1_1_2_1_neuron(num_inputs, hidden_dim)
        self.lif_1_1_2_1_layer2 = LIF_1_1_2_1_neuron(5*hidden_dim, hidden_dim)
    ...
    def forward(self, state):
        ...
        x = self.lif_1_1_2_1_layer1(state)
