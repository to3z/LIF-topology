...
class LIF_1_3_1_neuron(nn.Module):
    """
    1+3+1 LIF topology (mem-mixing, LIF_hh style).

    Slot layout: [batch, channel, 5]
        slot 0       -> 1 primary LIF (processes input)
        slots 1, 2, 3 -> 3 stage-2 mixing LIFs, see OLD mem[:,:,0] via
                         lif_fc1 = nn.Linear(1, 3) (non-negative)
        slot 4       -> 1 stage-3 mixing LIF, sees OLD (mem[:,:,1], mem[:,:,2],
                         mem[:,:,3]) via lif_fc2 = nn.Linear(3, 1) (non-negative)
    """
    def __init__(self, in_planes, out_planes):
        super(LIF_1_3_1_neuron, self).__init__()
    ...
    def update_neuron(self, input, mem, spike):
        ...
        input_all[:, :, 3] = inner1[:, :, 2]
        inner2 = self.lif_fc2(mem[:, :, 1:4])
        input_all[:, :, 4] = inner2[:, :, 0]
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, action_space=None):
        ...
        self.lif_1_3_1_layer1 = LIF_1_3_1_neuron(num_inputs, hidden_dim)
        self.lif_1_3_1_layer2 = LIF_1_3_1_neuron(5*hidden_dim, hidden_dim)
